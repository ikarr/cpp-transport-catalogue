...
int Node::AsInt() const {
    if (!IsInt()) { throw std::logic_error("Value isn't an integer"s); }
    return std::get<int>(*this);
}

double Node::AsDouble() const {
    if (!IsDouble()) { throw std::logic_error("Value isn't a double"s); }
    return IsPureDouble() ? std::get<double>(*this) : AsInt();
}

bool Node::AsBool() const {
    if (!IsBool()) { throw std::logic_error("Value isn't a boolean"s); }
    return std::get<bool>(*this);
}

const Array& Node::AsArray() const {
    if (!IsArray()) { throw std::logic_error("Value isn't an array"s); }
    return std::get<Array>(*this);
}
    
const std::string& Node::AsString() const {
    if (!IsString()) { throw std::logic_error("Value isn't a string"s); }
    return std::get<std::string>(*this);
}
    
const Dict& Node::AsDict() const {
    if (!IsDict()) { throw std::logic_error("Value isn't a map"s); }
...
template <>
void PrintValue<std::string>(const std::string& text, const PrintContext& ctx) {
    PrintString(text, ctx.out);
}
...
template <>
void PrintValue<Array>(const Array& array, const PrintContext& ctx) {
